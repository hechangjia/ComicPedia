...
export function GuideCharacterToggle({ checked, onChange, disabled = false }: GuideCharacterToggleProps) {
  return (
    <div className="rounded-xl border bg-amber-50/60 p-4 space-y-2">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <h3 className="text-sm font-medium text-amber-950">允许引导角色</h3>
        ...
        <label className={`relative inline-flex h-7 w-12 flex-shrink-0 items-center ${disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}>
          ...
          />
          <span className="absolute inset-0 rounded-full bg-amber-200 transition-colors peer-checked:bg-emerald-500" />
