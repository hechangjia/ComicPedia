import { buildExportPlan, defaultExportOptions, pdfGeometry, safeExportFilename, type ExportOptions } from "./options";
...
export async function renderPdfCoverPage(
  ...
  options: ExportOptions = defaultExportOptions("pdf"),
): Promise<HTMLCanvasElement> {
  const { widthPx: PDF_PAGE_W, heightPx: PDF_PAGE_H } = pdfGeometry(options.pdf);
  ...
  if (coverImage) {
    ...
    // Contain: do not crop artwork on the cover either.
    const scale = Math.min(imgAreaW / coverImage.width, imgAreaH / coverImage.height);
    const dw = coverImage.width * scale, dh = coverImage.height * scale;
    const dx = margin + (imgAreaW - dw) / 2, dy = imgY + (imgAreaH - dh) / 2;
  ...
  ctx.fillText(options.attribution || "Generated by Comicpedia", centerX, PDF_PAGE_H - Math.round(PDF_PAGE_H * 0.04));
...
export async function renderPdfPage(
  ...
  const { widthPx: PDF_PAGE_W, heightPx: PDF_PAGE_H, cols, rows, marginPx: margin } = pdfGeometry(options.pdf);
  const canvas = document.createElement("canvas");
  canvas.width = PDF_PAGE_W;
  canvas.height = PDF_PAGE_H;
  const ctx = canvas.getContext("2d")!;
  ...
  for (let i = 0; i < panels.length; i++) {
    ...
    if (panel.imageUrl) {
      try {
        ...
        const drawX = x + (cellW - drawW) / 2, drawY = y + (imgH - drawH) / 2;
        ctx.drawImage(img, drawX, drawY, drawW, drawH);
        ctx.restore();
      } catch (error) {
        throw new Error(`第 ${panelNumbers?.[i] ?? panelOffset + i + 1} 格图片读取失败：${error instanceof Error ? error.message : "请检查图片"}`);
      }
    }
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(String(panelNumbers?.[i] ?? panelOffset + i + 1), badgeCx, badgeCy);
  ...
  if (options.attribution) {
    ctx.fillStyle = "#666";
    ctx.font = `${Math.round(PDF_PAGE_W * 0.012)}px ${PDF_FONT_FAMILY}`;
    ctx.textAlign = "right";
    ctx.fillText(options.attribution, PDF_PAGE_W - margin, PDF_PAGE_H - margin / 2, PDF_PAGE_W - margin * 2);
  }
  return canvas;
}
 ...
 */
export async function downloadAsPdf(panels: ComicPanel[], title: string, options: ExportOptions = defaultExportOptions("pdf")): Promise<void> {
  const plan = buildExportPlan(panels, { ...options, format: "pdf" });
  const { jsPDF } = await import("jspdf");
  const geometry = pdfGeometry(options.pdf);
  const contentPages = Math.ceil(plan.entries.length / options.pdf.panelsPerPage);
  const totalPages = contentPages + (options.pdf.cover ? 1 : 0);
  const pdf = new jsPDF({ orientation: options.pdf.orientation, unit: "mm", format: options.pdf.paper, compress: true });
  const addCanvas = (canvas: HTMLCanvasElement) => {
    pdf.addImage(canvas.toDataURL("image/png"), "PNG", 0, 0, geometry.widthMm, geometry.heightMm);
    canvas.width = 0; canvas.height = 0;
  };
  if (options.pdf.cover) {
    const cover = await loadImage(plan.entries[0].panel.imageUrl!);
    addCanvas(await renderPdfCoverPage(title, "Comic", plan.entries.length, cover, options));
  }
  for (let page = 0; page < contentPages; page++) {
    if (options.pdf.cover || page > 0) pdf.addPage(options.pdf.paper, options.pdf.orientation);
    const offset = page * options.pdf.panelsPerPage;
    const entries = plan.entries.slice(offset, offset + options.pdf.panelsPerPage);
    addCanvas(await renderPdfPage(entries.map(entry => entry.panel), page + (options.pdf.cover ? 1 : 0), totalPages, title, page === 0, offset, options, entries.map(entry => entry.number)));
  }
  await pdf.save(`${safeExportFilename(title)}_${dateSuffix()}.pdf`, { returnPromise: true });
}
